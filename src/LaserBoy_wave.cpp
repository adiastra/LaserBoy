    ...
    parms             (                         )
{
    ...
    parms.max_dwell_millisec     = header.parms.max_dwell_millisec    ;
...
//############################################################################
LaserBoy_wave_header::LaserBoy_wave_header(std::fstream& in)
  : num_channels       (0),
    bits_per_sample    (0),
    num_samples        (0),
    sample_rate        (0),
...
//############################################################################
u_int LaserBoy_wave_header::lboy_chunk_size() const
{
...
//############################################################################
bool LaserBoy_wave_header::from_fstream_wave(std::fstream& in)
{
    char    a, b, c, d;
    ...
    if(temp_string == "LaserBoy")
    {
        ...
        if(LaserBoy_wave_mode & LASERBOY_WAVE_OPTIMIZED)
        {
            ...
            {   // floats were stored backwards. Oops!
                ...
                in.get(a); in.get(b); in.get(c); in.get(d);
                parms.max_dwell_millisec = (float)
                                           (   (a & 0x000000ff)
                                             | (b & 0x000000ff) << 8
                                             | (c & 0x000000ff) << 16
                                             | (d & 0x000000ff) << 24
        ...
        for(i = 0; i < num_channels; i++)
        {
            ...
            {
                case LASERBOY_SIGNAL_X_POSITION:             channel_map.push_back('X'); break;
                case LASERBOY_SIGNAL_Y_POSITION:             channel_map.push_back('Y'); break;
                case LASERBOY_SIGNAL_Z_POSITION:             channel_map.push_back('Z'); break;
                case LASERBOY_SIGNAL_RED_ANALOG:             channel_map.push_back('r'); break;
                case LASERBOY_SIGNAL_GREEN_ANALOG:           channel_map.push_back('g'); break;
                case LASERBOY_SIGNAL_BLUE_ANALOG:            channel_map.push_back('b'); break;
                ...
                case LASERBOY_SIGNAL_MONO_O_SCOPE:           channel_map.push_back('i'); break;
                case LASERBOY_NO_SIGNAL:                     channel_map.push_back('0'); break;
                default:                                     channel_map.push_back('?'); break;
            }
        }
        //----------------------------------------------------------------
...
//############################################################################
void LaserBoy_wave_header::to_fstream_wave(std::fstream& out) const
{
...
//----------------------------------------------------------------------------
// report information added for LaserBoy !!!
//----------------------------------------------------------------------------
    if(version != "!LaserBoy!")
    {
        out.put('L');
        ...
        if(LaserBoy_wave_mode & LASERBOY_WAVE_OPTIMIZED)
        {
            ...
            out.put((char)(((int)(parms.max_dwell_millisec) & 0xff000000) >> 24)); // float
            out.put((char)(((int)(parms.max_dwell_millisec) & 0x00ff0000) >> 16));
            out.put((char)(((int)(parms.max_dwell_millisec) & 0x0000ff00) >> 8 ));
            out.put((char) ((int)(parms.max_dwell_millisec) & 0x000000ff)       );
...
//----------------------------------------------------------------------------
    ...
    ((LaserBoy_wave_header*)this)->header_size = out.tellp();
    return;
}

//############################################################################
void LaserBoy_wave_header::tell(string label) const
{
    cout << "----------------------------------------------------------" << ENDL;
    ...
    cout << "       LaserBoy_wave_mode : " << LaserBoy_wave_mode << ENDL;
    return;
}
