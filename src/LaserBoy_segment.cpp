...
{   // mono spaced font constructor
    ...
    std::ifstream              char_list_file(font.substr(0, font.size() - 3) + "txt", ios::in ); // not binary!
    font_frames.from_ild_file(font, bytes_skipped, counter);
    getline(char_list_file, utf8_str);
    char_list_file.close();
    unicode_char_list = conv_utf8_utf32.from_bytes(utf8_str);
    //------------------------------------------------------------------------
...
//    else
    {
        int                   vertex_index;
        double                length = p0 | p1;
        LaserBoy_real_segment real_segment(font_frames,
                                           unicode_char_list,
                                           text,
        ...
        for(vertex_index = 0; vertex_index < (int)real_segment.size(); vertex_index++)
        {
            push_back(LaserBoy_vertex(   real_segment[vertex_index]
                                       * (length / real_segment.width())
                                       + p0
                                     )
                     );
            if(real_segment[vertex_index].is_color())
            {
            ...
            else if(real_segment[vertex_index].is_black(p_space->black_level))
            {
                  ...
                  segment_error          (LASERBOY_OK)
{
    ...
    if(    ratio > 0
        ...
        arc_angle = roller_radius / greatest_common_devisor(fixed_radius, roller_radius) * 360;
    total_vertices = (int)ceil(arc_angle / p_space->rendered_arc_angle);
    if(total_vertices > 15000) // that's a lot!
        total_vertices = 15000;
    arc_step = arc_angle / total_vertices;
                  ...
                  segment_error          (LASERBOY_OK)
{
    ...
    vertex.g = p_space->palette_picker(p_space->palette_index)[vertex.c].g;
    vertex.b = p_space->palette_picker(p_space->palette_index)[vertex.c].b;
    vertex.x = (short)((fixed_radius - roller_radius) + roller_offset);
    vertex.y = 0;
    vertex.z = center.z;
    push_back(LaserBoy_vertex(vertex + center));
    vertex.unblank();
    for(u_int a = 0; a <= total_vertices; a++)
    {
        vertex.x = (short)(   (fixed_radius - roller_radius) * cos(a * arc_step)
                            + roller_offset * cos((ratio - 1) * a * arc_step)
                          );
        vertex.y = (short)(   (fixed_radius - roller_radius) * sin(a * arc_step)
                            - roller_offset * sin((ratio - 1) * a * arc_step)
                          );
        vertex.z = center.z;
        push_back(LaserBoy_vertex(vertex + center));
                  ...
                  segment_error          (LASERBOY_OK)
{
    ...
    total_vertices = (int)ceil(arc_angle / p_space->rendered_arc_angle);
    if(total_vertices > 30000) // that's a lot!
        total_vertices = 30000;
    arc_step = arc_angle / total_vertices;
    arc_step *= one_degree; // convert to radians!
    phase_x  *= one_degree;
    phase_y  *= one_degree;
...
//############################################################################
LaserBoy_segment::LaserBoy_segment(LaserBoy_oscillator LBO1,
                ...
                : vector<LaserBoy_vertex>(),
                  ...
                  segment_error          (LASERBOY_OK)
{
    LaserBoy_real_segment rs = LaserBoy_real_segment::harmonograph(LBO1,
                                                                   LBO2,
                                                                   LBO3,
                                                                   LBO4,
                                                                   LBO1,
        ...
        if(bmp->bpp <= 8)
            for(i = 1; i < size(); i++)
            {
                ...
                color = bmp->get_pixel(  bmp,
                                         (vertex.x + 32767) / factor - x_off,
                                         (vertex.y + 32767) / factor - y_off
                                      );
...
//############################################################################
void LaserBoy_segment::to_fstream_wave(std::fstream&         out,
