...
//############################################################################
LaserBoy_frame::LaserBoy_frame(LaserBoy_segment& segment)
                 : LaserBoy_ild_header ()
                 ...
                 , coda                ()
     {
          intro.palette_index = segment.palette_index;
         bridge.palette_index = segment.palette_index;
           coda.palette_index = segment.palette_index;
        if(segment.palette_index > LASERBOY_TRUE_COLOR)
            palette_name = p_space->palette_picker(segment.palette_index).name;
        else
            palette_name.clear();
         is_2D();
     }
...
//############################################################################
LaserBoy_frame::LaserBoy_frame(const int& palette_index,
                               bool       add_origin
                              )
                 : LaserBoy_ild_header ()
                 , LaserBoy_segment    (palette_index, add_origin)
                 , is_selected         (false)
                 , is_unique           (true)
                 , is_wagged           (false)
                 , egg                 (0)
                 ...
                 , coda                ()
     {
          intro.palette_index = palette_index;
         bridge.palette_index = palette_index;
           coda.palette_index = palette_index;
        if(palette_index > LASERBOY_TRUE_COLOR)
            palette_name = p_space->palette_picker(palette_index).name;
        else
            palette_name.clear();
     }

//############################################################################
LaserBoy_frame::LaserBoy_frame(const LaserBoy_ild_header& header,
...
LaserBoy_frame::LaserBoy_frame(LaserBoy_vertex(*F)(int, int),
               ...
               , coda               (     )
{
    palette_name           = p_space->palette_picker(LASERBOY_LASERBOY_HUES).name;
    is_selected            = false;
    is_unique              = true;
    is_wagged              = false;
...
//############################################################################
void LaserBoy_frame::sync_rgb_and_palette()
{
    LaserBoy_segment::sync_rgb_and_palette();
    if(palette_index > LASERBOY_TRUE_COLOR)
        palette_name = p_space->palette_picker(palette_index).name;
    else
        palette_name.clear();
...
void LaserBoy_frame::add_coda(LaserBoy_3D_short next_frame_entry_point)
{
    ...
    else // coda must exist to convey end_of_frame in optimized wav!
    {
       coda.push_back(black_vertex);
       coda.push_back(black_vertex);
    }
...
{
    LaserBoy_vertex  point_of_entry; // origin
    LaserBoy_segment entry;
    //------------------------------------------------------------------------
    if(p_space->maintain_first_anchor)
        point_of_entry = first_lit_anchor();
    //------------------------------------------------------------------------
    if(frame_index <= 0)
    {
...
void LaserBoy_frame::to_ofstream_ctn(std::ofstream& out)
{
    ...
    if(copy.size() > 1)
    {
        if(p_space->auto_opti_ctn)
        {
            ...
            copy.optimize(0);
            //-----------------------------------------------------------------
            for(j = 0; j < (u_int)round(p_space->dwell_on_dot); j++)
...
//############################################################################
bool LaserBoy_frame::save_as_wav(const string& file, bool optimized, bool timed, bool clip)
{
    ...
    if(out.is_open() && size() > 1)
    {
        ...
        if(optimized)
        {
            double          max_wag_microsec_was = p_space->max_wag_microsec;
            ...
            copy.to_fstream_wav(out, header, true, timed, clip);
            p_space->max_wag_microsec = max_wag_microsec_was;
        }
        else
        {
            header.to_fstream_wav(out); // take up the space!
            copy.to_fstream_wav(out, header, false, timed, false);
        }
        //----------------------------------------------------------------
        header.to_fstream_wav(out); // rewrite the header with the correct chunk sizes
        //----------------------------------------------------------------
        out.close();
        return true;
    }
...
//############################################################################
void LaserBoy_frame::to_fstream_wav(std::fstream&        out,
                                    LaserBoy_wav_header& header,
                                    bool                 optimized,
                                    bool                 timed,
                                    bool                 clip
                                   )
{
    if(optimized)
    {
        ...
        //----------------------------------------------------------------
        p_space->p_GUI->display_state("saving frame as wav");
        //----------------------------------------------------------------
        if(timed) // still_frame_duration
        {
            ...
            intro.to_fstream_wav(out, header, false, false);
            LaserBoy_segment::to_fstream_wav(out, header, is_wagged, is_wagged);
            if(total_frame_scans > 1)
            {
                for(int frame_scan = 1; frame_scan < total_frame_scans; frame_scan++)
                {
                    bridge.to_fstream_wav(out, header, true, (!is_wagged) && (frame_scan == 1)); // frame scan 1 is unique
                    LaserBoy_segment::to_fstream_wav(out, header, is_wagged && (frame_scan < (total_frame_scans - 1)), false);
                    p_space->p_GUI->display_progress(total_frame_scans - frame_scan);
                }
                coda.to_fstream_wav(out, header, false, false);
            }
            chill.to_fstream_wav(out, header, true, total_frame_scans == 1);
        } // end if(timed)
        else // single frame
        {
            if(clip)
            {
                bridge.to_fstream_wav(out, header, false, false);
                LaserBoy_segment::to_fstream_wav(out, header, true, true);
            }
            else // with lead and tail
            {
                chill.to_fstream_wav(out, header, false, false);
                intro.to_fstream_wav(out, header, false, false);
                LaserBoy_segment::to_fstream_wav(out, header, false, false);
                coda.to_fstream_wav(out, header, false, false);
                chill.to_fstream_wav(out, header, true, true);
            }
        }
    } // end if(optimized)
    //------------------------------------------------------------------------
    else if(timed) // no intro, bridge or coda!
    {
        ...
        for(int frame_scan = 1; frame_scan < total_frame_scans; frame_scan++)
        {
            LaserBoy_segment::to_fstream_wav(out, header, true, false);
            p_space->p_GUI->display_progress(total_frame_scans - frame_scan);
        }
    } // end else if(timed)
    //------------------------------------------------------------------------
