...
public:
   ...
   ~LaserBoy_3D_short()
    ...
    u_int  (*put_pixel    )(struct LaserBoy_bmp* bmp, int x, int y, u_int c);
    u_int  (*get_pixel    )(struct LaserBoy_bmp* bmp, int x, int y);
    u_int  (*get_pixel_rgb)(struct LaserBoy_bmp* bmp, int x, int y);
    //------------------------------------------------------------------------
    ...
    LaserBoy_3D_short  operator - ()
    {
        ...
        if(x < LASERBOY_MIN_SHORT)
            negative.x = LASERBOY_MAX_SHORT;
        else
            negative.x = -x;
        if(y < LASERBOY_MIN_SHORT)
            negative.y = LASERBOY_MAX_SHORT;
        else
            negative.y = -y;
        if(z < LASERBOY_MIN_SHORT)
            negative.z = LASERBOY_MAX_SHORT;
        else
            negative.z = -z;
        return negative;
    }
    ...
    //------------------------------------------------------------------------
    LaserBoy_3D_short abs_3D()
    {
        return LaserBoy_3D_short(abs(x), abs(y), abs(z));
    }
    //------------------------------------------------------------------------
    int sum()
    {
